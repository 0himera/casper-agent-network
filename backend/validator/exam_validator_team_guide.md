...
Before first prod dispatch, the 3 **required MVP** exam templates must have human reviewer notes on file. Optional templates (`exam-curve-3pool-tvl-block-19000000`, `exam-rwa-tokenized-tbill-nav-2024-q3`) do not block this gate.
...
| `notes` | Short free-text (e.g. leak check, E0 format) |

**Permanent record:** `backend/validator/documentation/exam_idea_implementation.md` § E5.5 «MVP seed review notes».

**Second reviewer:** For the current MVP release, the E5.5 second-reviewer sign-off is **waived by policy** (see `exam_idea_implementation.md` § E5.5 «Second reviewer waiver»). A second reviewer is not required before prod dispatch.

**Do not dispatch to prod** until all 3 required rows are recorded and `matches_canonical = yes` for each (or canonical values are corrected in `seed_exam_pool.sql` first). This blocking condition does **not** require a second reviewer sign-off.
...
Recorded E6 smoke result: see `documentation/exam_e6_recommendation.md` for the latest manual live-LLM run. Current recorded outcome is `3/3` matching human labels on the real-smoke subset (dev endpoint, 2026-06-26). **Before target-environment enablement**, re-run smoke on the target provider/model and record the result in the decision doc.

### E6 rollout checklist (target provider)

1. Run mock regression (CI-safe):
   ```bash
   cd backend/validator
   cargo test --lib exam_equality
   cargo test --lib exam::orchestrator
   cd ../
...
4. Record result in `documentation/exam_e6_recommendation.md` (enablement decision table).
5. Only then set `EXAM_LLM_EQUALITY=1` in the target backend `.env`.

**Failure checks:** missing credentials → smoke script exits 1; timeout → manual check with `VALIDATOR_JUDGE_TIMEOUT_MS=1`; automated fail-closed tests cover missing provider, unparseable/empty LLM response (see decision doc rollout brief).
